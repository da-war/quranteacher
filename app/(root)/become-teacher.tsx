...
import {
  becomeTeacherInitialValues,
  becomeTeacherValidationSchema,
} from "@/lib/schema";
...
const BecomeTeacher = () => {
  ...
  const uploadImage = async (uri: string, name: string) => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error instanceof Error ? error.message : "Can't upload files!"
      );
      return;
    } finally {
  ...
  const applyAsTeacher = async (values: BecomeTeacherFormValues) => {
    ...
    if (userId) {
      try {
        ...
        const cnicFrontUrl = values.cnicFront
          ? await uploadImage(values.cnicFront, new Date().getTime().toString())
          : null;
        const cnicBackUrl = values.cnicBack
          ? await uploadImage(values.cnicBack, new Date().getTime().toString())
          : null;
        const certificatesUrls = await Promise.all(
          values.certificates.map(
            async (cert) =>
              await uploadImage(cert, new Date().getTime().toString())
          )
        );
    ...
    <>
      <SafeAreaView style={styles.container}>
            ...
            >
              <View>
                ...
                <Text className="text-md text-center font-JakartaBold mt-3 mb-7 text-neutral-500">
                  Fill the form below to apply as a teacher, after that our team
                  will review your application verify details manually and get
                  back to you!
                </Text>
              </View>
              <AppForm
                ...
                onSubmit={(values: BecomeTeacherFormValues) =>
              ...
              >
                ...
                <AppFormFieldList
                  ...
                  placeholder="BS IT, MS Islamiat, F.A etc"
                />
                <AppFormRadioButton
                  name="gender"
                  title="Gender"
                ...
                <Text className="text-lg font-JakartaBold text-center mx-auto my-7">
                  Complete the details to get verified{" "}
                ...
                <View className="my-2">
                  <MdText
                    text="Upload Certificates/Degrees"
                    subText="(optional up to 5)"
                  />
                ...
                <SubmitButton
                  ...
                  className="rounded-md my-5 bg-primary-500"
                />
              </AppForm>
            </ScrollView>
          </KeyboardAvoidingView>
        ...
        {user?.isTeacherApplied && (
          <View className="flex-1 p-4 justify-center items-center">
            <Text className="text-xl font-JakartaSemiBold text-center w-[80%]">
              Already Applied as a teacher we'll get back to you shortly
            </Text>
          </View>
        )}
      </SafeAreaView>
      ...
      <Modal visible={loading}>
        <View className="flex-1 justify-center items-center">
          ...
          <Text className="text-2xl font-JakartaSemiBold text-center">
            Uploading Data...
          </Text>
        </View>
      </Modal>
